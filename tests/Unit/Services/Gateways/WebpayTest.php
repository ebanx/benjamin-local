...
use Ebanx\Benjamin\Services\Gateways\Webpay;
use Ebanx\Benjamin\Services\Http\Client;

class WebpayTest extends GatewayTestCase
{
    public function testPayment()
    {
        $webpaySuccessfulResponse = $this->getWebpaySuccessfulResponseJson();
        $client = $this->getMockedClient($webpaySuccessfulResponse);

        $factory = new BuilderFactory('es_CL');
        $payment = $factory->payment()->build();
        $gateway = new Webpay($this->config, $client);
